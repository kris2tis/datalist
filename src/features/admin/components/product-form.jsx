...
export default function ProductForm({ mode, data }) {
  const {
    handleSubmit,
    register,
    control,
  ...
  const handleCreateProduct = async (e) => {
    ...
    // const data = { ...e };
    const createProductFormData = { ...e };
    console.log(createProductFormData);
    try {
      setIsLoading(true);
      const { message } = await http
        .post("/admin/product", createProductFormData)
        .then(({ data }) => data);
  ...
  const handleEditProduct = async (e) => {
    const editProductFormData = { productId: data.productId, ...e };
    try {
      setIsLoading(true);
      const { message } = await http
        .put("/admin/product", editProductFormData)
        .then(({ data }) => data);
      toast.success(message);
      setIsLoading(false);
      push("/admin/product");
  ...
  const fillProductInput = () => {
    reset({
      ...
      quantity: 1,
      properties: [
        { name: "تست 1", value: "تست 1" },
        { name: "تست2", value: "تست 2" },
      ],
    });
  };
  return (
            ...
            <div className="flex items-center gap-3">
              ...
              {mode === "create"
                ? process.env.NODE_ENV === "development" && (
                    <button type="button" onClick={fillProductInput}>
                      پر کردن فیلد ها{" "}
                    </button>
                  )
                : null}
